...
func (a *App) handlePost(w http.ResponseWriter, r *http.Request, p string) {
	...
	case "/api/model":
		...
		if err != nil || !DirExists(model) {
			a.jsonCode(w, map[string]any{"error": "model directory does not exist"}, http.StatusBadRequest)
			return
		}
		if a.busyH3() {
			a.jsonCode(w, map[string]any{"error": "stop the active h3 process before changing the model"}, http.StatusConflict)
			return
		}
		a.cfg.Model = model
		_ = WriteJSONFile(a.cfg.ModelFile, map[string]any{"model": model}, true)
		a.json(w, map[string]any{"model": model})
	case "/api/h3":
		h3, err := filepath.Abs(expandHome(anyToString(data["h3"])))
		info, statErr := os.Stat(h3)
		if err != nil || statErr != nil || info.IsDir() || info.Mode()&0o111 == 0 {
			a.jsonCode(w, map[string]any{"error": "h3 executable does not exist or is not executable"}, http.StatusBadRequest)
			return
		}
		if a.busyH3() {
			a.jsonCode(w, map[string]any{"error": "stop the active h3 process before changing the binary"}, http.StatusConflict)
			return
		}
		a.cfg.H3 = h3
		a.cfg.Workdir = filepath.Dir(h3)
		_ = WriteJSONFile(a.cfg.H3File, map[string]any{"h3": h3}, true)
		a.json(w, map[string]any{"h3": h3})
...
func (a *App) upload(w http.ResponseWriter, r *http.Request) {
	...
	for _, entry := range inputs {
		if anyToString(entry["name"]) == filepath.Base(dst) {
			item = entry
			break
		}
	}
	response := map[string]any{"name": filepath.Base(dst), "kind": nil, "duration": nil, "inputs": inputs}
	if item != nil {
		response["kind"] = item["kind"]
		if value, ok := item["duration"]; ok {
